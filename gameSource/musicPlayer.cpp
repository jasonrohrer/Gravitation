...
Timbre *musicTimbres[ 6 ];

int numEnvelopes = 6;

Envelope *musicEnvelopes[ 6 ];
...
void audioCallback( void *inUserData, Uint8 *inStream, int inLengthToFill ) {
    ...
    for( int n=0; n<currentlyPlayingNotes.size(); n++ ) {
        ...
        if( trackFadeInLevel >= note->mTimbreNumber ) {
            // full volume
            thisTrackLevel = 1.0;
            }
        else if( trackFadeInLevel > note->mTimbreNumber - 1 ) {
...
void loadMusicImage( const char *inTGAFileName ) {
    ...
    musicTimbres[0] = new Timbre( sampleRate, 0.15 * loudnessPerTimbre,
                                  keyFrequency / 4,
                                  heightPerTimbre, squareWave );
    musicTimbres[1] = new Timbre( sampleRate, loudnessPerTimbre,
                                  keyFrequency,
                                  heightPerTimbre, sin );
    musicTimbres[2] = new Timbre( sampleRate, 0.35 * loudnessPerTimbre,
                                  keyFrequency / 2,
                                  heightPerTimbre, sawWave );
    musicTimbres[3] = new Timbre( sampleRate, 0.5 * loudnessPerTimbre,
                                  keyFrequency / 4,
                                  heightPerTimbre, smoothedWhiteNoise );
    musicTimbres[4] = new Timbre( sampleRate, loudnessPerTimbre,
                                  keyFrequency * 2,
                                  heightPerTimbre, sin );
    musicTimbres[5] = new Timbre( sampleRate, 0.65 * loudnessPerTimbre,
                                  keyFrequency,
                                  heightPerTimbre, sawWave );
    ...
    musicEnvelopes[0] = new Envelope( 0.25, 0.0, 1.0, 0.1,
                                      maxNoteLength,
                                      gridStepDurationInSamples );
    musicEnvelopes[1] = new Envelope( 0.1, 0.9, 0.0, 0.0,
                                      maxNoteLength,
                                      gridStepDurationInSamples );
    musicEnvelopes[2] = new Envelope( 0.05, 0.7, 0.25, 0.1,
                                      maxNoteLength,
                                      gridStepDurationInSamples );
    musicEnvelopes[3] = new Envelope( 0.0, 0.2, 0.0, 0.0,
                                      maxNoteLength,
                                      gridStepDurationInSamples );
    musicEnvelopes[4] = new Envelope( 0.1, 0.9, 0.0, 0.0,
                                      maxNoteLength,
                                      gridStepDurationInSamples );
    musicEnvelopes[5] = new Envelope( 0.05, 0.7, 0.25, 0.1,
                                      maxNoteLength,
                                      gridStepDurationInSamples );
